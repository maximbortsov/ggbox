...
transaction(recipientAddress: Address, editionIDs: [UInt64], counts: [UInt64], metadatas: [{String:String}]) {
    ...
    execute {
        ...
        while i < editionIDs.length {
            ...
            while remaining > 0 {
                ...
                self.recipient.deposit(token: <- self.minter.mintNFT(editionID: editionIDs[i], metadata: metadatas[i]))
